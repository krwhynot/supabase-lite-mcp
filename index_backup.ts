...
class SupabaseLiteMCPServer {
  ...
  private async listTables(schemas: string[]): Promise<any> {
    const query = `
      SELECT 
  ...
  private async listExtensions(): Promise<any> {
    ...
    const { data, error } = await this.supabase.rpc('query', { query });
  ...
  private async runSecurityChecks(): Promise<any[]> {
    ...
    const { data } = await this.supabase.rpc('query', { query: rlsQuery });
  ...
  private async runPerformanceChecks(): Promise<any[]> {
    ...
    const { data } = await this.supabase.rpc('query', { query: indexQuery });
